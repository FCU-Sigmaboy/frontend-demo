                ...
                <div class="rating-overview">
                  ...
                  <div class="rating-stars-large">
                    <i
                      ...
                      :class="[
                        'bi',
                        n <= Math.floor(reviewStore.averageRating) ? 'bi-star-fill' : 'bi-star',
                      ]"
                    ></i>
                  </div>
      ...
      <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
          <div class="modal-header border-0">
            <h5 class="modal-title w-100 text-center" id="badgeModalLabel">
              {{ selectedBadge?.label }}
            </h5>
            <button
              type="button"
          ...
          <div class="modal-body text-center">
            <div v-if="selectedBadge">
              ...
              <div
                v-else-if="selectedBadge.type === 'trophy' && selectedBadge.icon"
                class="trophy-icon-large mb-3"
              >
                {{ selectedBadge.icon }}
              </div>
              ...
              <div v-else class="badge-status locked">
                <div class="progress-info">
                  <p class="mb-2">
                    <strong>目前進度：{{ Math.round(selectedBadge.progress) }}%</strong>
                  </p>
                  ...
                  <p
                    v-if="selectedBadge.remainingKg !== undefined && selectedBadge.remainingKg > 0"
                    class="text-muted small mb-0"
                  >
                    還需
                    <strong class="text-primary"
                      >{{ selectedBadge.remainingKg?.toFixed(1) || '0.0' }} kg</strong
                    >
                    即可解鎖
                  </p>
                  <p v-else-if="selectedBadge.threshold" class="text-muted small mb-0">
                    需達成
                    <strong class="text-primary">{{ selectedBadge.threshold }}</strong> 即可解鎖
                  </p>
                </div>
              </div>
            </div>
...
<script setup>
  ...
  const activeListings = computed(() => {
    return myListings.value
      .filter((item) => item.listing_status === true)
      .sort((a, b) => new Date(b.created_at) - new Date(a.created_at))
  })

  const inactiveListings = computed(() => {
    return myListings.value
      .filter((item) => item.listing_status === false)
