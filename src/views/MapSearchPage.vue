  ...
  <div class="map-search-page">
    ...
    <div v-else-if="!state.userLocation" class="no-location-prompt">
      <div class="alert alert-warning">
        ...
        <button class="btn btn-primary" @click="initialize">重新嘗試</button>
      </div>
    </div>
    ...
    <div v-else class="map-content-wrapper">
      ...
      <div class="location-switcher">
        <button class="location-btn" @click="toggleLocationMenu">
          ...
          <span class="location-text">
            {{
              state.currentLocationType === 'current'
                ? '目前位置'
                : state.currentLocationType === 'home'
                  ? '家'
                  : '公司'
            }}
          </span>
        ...
        <div v-if="state.showLocationMenu" class="location-menu">
          ...
          <button
            class="location-option"
            :class="{
              active: state.currentLocationType === 'home',
          ...
          >
            ...
            <span>家</span>
            <span v-if="!state.savedLocations.home" class="not-set">{{
              authStore.user ? '(未設定)' : '(未登入)'
            }}</span>
          </button>
          <button
            class="location-option"
            :class="{
              active: state.currentLocationType === 'work',
          ...
          >
            ...
            <span>公司</span>
            <span v-if="!state.savedLocations.work" class="not-set">{{
              authStore.user ? '(未設定)' : '(未登入)'
            }}</span>
          </button>
        </div>
      ...
      <!-- Back Button -->
      <button class="view-toggle-btn" @click="toggleToListView">
...
<script setup>
  ...
  function parseWKBPoint(wkbHex) {
    try {
      ...
      console.log(
        '[MapSearchPage] DEBUG - WKB hex length:',
        wkbHex.length,
        'hex:',
        wkbHex.substring(0, 40)
      )
      ...
      if (!lonHex || !latHex || lonHex.length !== 16 || latHex.length !== 16) {
        console.error('[MapSearchPage] Invalid hex string lengths:', {
          lonHex: lonHex?.length,
          latHex: latHex?.length,
        })
  ...
  async function fetchSavedLocations() {
    try {
      ...
      console.log(
        '[MapSearchPage] DEBUG - Raw locations from API:',
    ...
    switch (locationType) {
      ...
      case 'home':
        if (state.savedLocations.home) {
          ...
          if (
            state.savedLocations.home.latitude !== null &&
            state.savedLocations.home.longitude !== null
          ) {
      ...
      case 'work':
        if (state.savedLocations.work) {
          // Validate coordinates before switching
          if (
            state.savedLocations.work.latitude !== null &&
            state.savedLocations.work.longitude !== null
          ) {
  ...
  async function handleSearch(searchParams) {
    ...
    state.filters.distance_range_km =
      searchParams.distance !== '' && searchParams.distance !== null
  ...
  async function handleCategoryFilter() {
    ...
    const activeTab = document.querySelector(
      '.filter-tabs-section:not(.sub-category-tabs) .filter-tab.active'
    )
  ...
  async function handleSubCategoryFilter() {
    ...
    if (activeTab) {
      ...
      if (activeFilter) {
        ...
        console.log(
          '[MapSearchPage] Updated sub-category filter to:',
  ...
  async function initialize() {
    ...
    try {
      ...
      if (
        ...
        state.filters.main_category_id ||
        state.filters.sub_category_id
      ) {
        state.showSearchResults = true
      }
  ...
  watch(
    ...
    { deep: false }
  ) // Use shallow watch to only track reference changes
