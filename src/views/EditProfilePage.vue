        ...
        <div class="form-card">
          ...
          <form v-else @submit.prevent="handleSubmit">
            ...
            <div class="form-section">
              ...
              <div class="hint-row">
                <p class="field-hint">
                  Google 帳戶預設顯示名稱，可隨時修改（最多 {{ maxNicknameLength }} 字）
                </p>
            ...
            <div class="form-section">
              <label for="loginMethod" class="form-label"> 登入方式 </label>
              <input
                id="loginMethod"
            ...
            <!-- Location Field -->
            <div class="form-section">
              <label for="location" class="form-label"> 所在地區 </label>
            ...
            <div v-if="showOfficeAddress" class="form-section">
              <label for="officeAddress" class="form-label"> 公司地址 </label>
              <div class="location-input-group">
                <input
                  id="officeAddress"
...
<script setup>
  ...
  const districtCoordinates = {
    ...
    北屯區: { latitude: 24.181, longitude: 120.715 },
    豐原區: { latitude: 24.2569, longitude: 120.723 },
    大里區: { latitude: 24.099, longitude: 120.6772 },
  ...
  const getCurrentLocation = async () => {
    ...
    try {
      ...
      if (address) {
        ...
        const district =
          address.town || address.city_district || address.district || address.suburb || ''
    ...
    try {
      ...
      if (profile) {
        ...
        if (profile.locations && profile.locations.length > 0) {
          ...
          const officeAddr = profile.locations.find(
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (formData.value.avatarFile) {
        ...
        const avatarUrl = await uploadProfilePicture(formData.value.avatarFile, authStore.user?.id)
