        ...
        <div class="modal-container">
          ...
          <div v-else-if="itemDetail" class="modal-content">
            ...
            <div class="modal-body">
              <!-- Image Gallery -->
              <div
                v-if="itemDetail.image_urls && itemDetail.image_urls.length > 0"
                class="image-gallery"
              >
              ...
              <div class="item-info">
                ...
                <div v-if="sellerId" class="seller-summary">
                  ...
                  >
                    <i
                      :class="isFollowing ? 'bi bi-person-check-fill' : 'bi bi-person-plus-fill'"
                    ></i>
                  </button>
                </div>
                ...
                <div class="meta-info">
                  <div
                    v-if="itemDetail.distance_km !== null && itemDetail.distance_km !== undefined"
                    class="meta-item"
                  >
                ...
                <div v-if="itemDetail.description" class="description-section">
                  ...
                  >
                    ...
                    <i
                      :class="isDescriptionExpanded ? 'bi bi-chevron-up' : 'bi bi-chevron-down'"
                    ></i>
                  </button>
                </div>
...
<script setup>
  ...
  const sellerId = computed(
    () => itemDetail.value?.seller?.id || itemDetail.value?.user?.id || null
  )
  const isOwner = computed(
    () => authStore.user && sellerId.value && authStore.user.id === sellerId.value
  )
  const sellerName = computed(
    () => itemDetail.value?.seller?.nickname || itemDetail.value?.user?.nickname || '賣家'
  )
  const sellerAvatar = computed(
    () =>
      itemDetail.value?.seller?.profile_picture_url ||
      itemDetail.value?.user?.profile_picture_url ||
      'https://placehold.co/48/6fb8a5/ffffff?text=U'
  )
  ...
  function syncEngagementStates() {
    ...
    isFavorite.value =
      !!itemDetail.value.favorited_at ||
      favoritesStore.isFavorite(itemDetail.value.item_id || itemDetail.value.id)
    isFollowing.value = !!(
      itemDetail.value.seller?.followed_at || itemDetail.value.user?.followed_at
    )
  }
  ...
  watch(
    () => [props.modelValue, props.itemId, props.userLocation],
    ([isOpen, newItemId, newLocation]) => {
      if (isOpen && newItemId) {
        loadItemDetails()
      }
    },
