...
<script setup>
  ...
  async function renderSearchRadius() {
    ...
    if (props.searchRadius !== null && props.searchRadius !== undefined) {
      radiusCircle.value = L.circle([props.userLocation.latitude, props.userLocation.longitude], {
  ...
  function getCategoryIcon(item) {
    ...
    const mainCategory = categoriesStore.mainCategories.find(
      (cat) => cat.id === item.main_category_id
    )
    ...
    locationGroups.forEach((group) => {
      ...
      markerPositions.forEach((position) => {
        ...
        const profilePicture =
          firstItem.user?.profile_picture_url ||
          'https://placehold.co/40/1e1e1e/ffffff?text=' +
            (firstItem.user?.nickname?.charAt(0) || 'U')
  ...
  function recenterMap() {
    if (map.value && props.userLocation) {
      map.value.setView([props.userLocation.latitude, props.userLocation.longitude], props.zoom)
    }
  }
