      ...
      <BContainer fluid class="px-0">
        <div class="header-content">
          ...
          <div class="header-right">
            ...
            <template v-if="authStore.isLoggedIn">
              ...
              >
                ...
                <div class="transaction-badges" aria-hidden="true">
                  <span v-if="pendingConfirmationCount > 0" class="transaction-badge badge-pending">
                    {{ pendingConfirmationCount > 99 ? '99+' : pendingConfirmationCount }}
                  </span>
                  <span v-if="inTransactionCount > 0" class="transaction-badge badge-active">
              ...
              <div class="mobile-user-section d-md-none">
                ...
                <i
                  v-else
                  class="bi bi-person-circle mobile-avatar-icon"
                  @click="router.push({ name: 'UserProfile' })"
                ></i>
              </div>
            </template>
            ...
            <BButton variant="link" class="hamburger-button d-md-none" @click="toggleUnifiedMenu">
              ...
              <div v-for="cat in filteredCategories" :key="cat.id" class="category-section">
                <div class="category-item expandable" @click="toggleCategory(cat.id)">
                  ...
                  <i
                    :class="[
                      'bi',
                      expandedCategories.includes(cat.id) ? 'bi-chevron-up' : 'bi-chevron-down',
                    ]"
                    class="expand-icon"
                  ></i>
                </div>
              ...
              <ul class="user-menu-list">
                ...
                <li @click="handleMenuAction('TransactionRecords')" class="menu-item-with-badge">
                  ...
                  >
                    ...
                    <span v-if="inTransactionCount > 0" class="menu-transaction-badge badge-active">
                      {{ inTransactionCount > 99 ? '99+' : inTransactionCount }}
                    </span>
                  </span>
                </li>
...
<script setup>
  ...
  const inTransactionCount = computed(
    () =>
      (transactionStore.pending.giver?.length ?? 0) +
      (transactionStore.pending.receiver?.length ?? 0)
  )
    ...
    return categories.value
      .map((cat) => {
        ...
        const matchedSubcategories =
          cat.sub_categories?.filter((sub) => sub.name.toLowerCase().includes(searchLower)) || []
