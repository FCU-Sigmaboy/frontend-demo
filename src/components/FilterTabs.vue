    ...
    if (currentFilter.sortKey) {
      ...
      items.sort((a, b) => {
        ...
        if (
          ...
          return sortOrder.value[currentFilter.id] === 'asc' ? dateA - dateB : dateB - dateA
        } else if (typeof valueA === 'number' || !isNaN(parseFloat(valueA))) {
          // 數字類型
          const numA = parseFloat(valueA) || 0
          const numB = parseFloat(valueB) || 0
          return sortOrder.value[currentFilter.id] === 'asc' ? numA - numB : numB - numA
        } else {
          // 字串類型
