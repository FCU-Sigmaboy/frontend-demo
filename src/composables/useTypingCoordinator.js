...
export function useTypingCoordinator({
  ...
  function currentConversation() {
    try {
      return typeof getSelectedConversationId === 'function'
        ? getSelectedConversationId()
        : null;
    } catch (err) {
  ...
  function broadcastTyping(conversationId, isTyping, identity) {
    ...
    return messageStore
      .broadcastTypingStatus(targetConversation, isTyping, actor)
      .catch(err => {
        console.error('[TypingCoordinator] failed to broadcast typing status', err);
      });
  }

  async function handleConversationChanged({ prevConversationId = null, nextConversationId = null, identity = null } = {}) {
