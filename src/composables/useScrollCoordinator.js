...
export function useScrollCoordinator({
  ...
  async function handleMetrics(payload = {}) {
    ...
    const rawScrollHeight =
      typeof payload.scrollHeight === 'number' ? payload.scrollHeight : area.scrollHeight
    const rawClientHeight =
      typeof payload.clientHeight === 'number' ? payload.clientHeight : area.clientHeight
    ...
    if (atBottom) {
      ...
      if (!wasPreviouslyAtBottom && selectedConversation.value) {
        const hasUnreadMessages = messageStore.currentMessages.some(
