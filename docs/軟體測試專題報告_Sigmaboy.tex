...
% Load xcolor with necessary options for named colors
...
% --- OTHER PACKAGES ---
% --- OTHER PACKAGES ---
% --- COMPATIBILITY POLYFILL (SHOTGUN) ---
% 1. Define missing structure commands as simple no-ops to preserve TOC content
\makeatletter
\long\def\NewStructureName#1{}
\long\def\AssignStructureRole#1#2{}
\long\def\NewTaggingSocket#1#2{}
\long\def\NewTaggingSocketPlug#1#2#3{}
\long\def\AssignTaggingSocketPlug#1#2{}
\long\def\UseStructureName#1{}
% Use simple def for body commands to avoid spacing/swallowing issues of bsphack
\long\def\tagstructbegin#1{}
\long\def\tagstructend{}
\long\def\tagpdfparaOff#1{}

% 2. Pre-allocate missing integer variables to prevent tcolorbox crash
% We define multiple naming variants because the exact mangling of '/' is uncertain
\expandafter\newcount\csname g__tag_support_tcb/box/init_args_int\endcsname
\expandafter\newcount\csname g__tag_support_tcb_box_init_args_int\endcsname
\expandafter\newcount\csname g__tag_support_tcb/box_init_args_int\endcsname
\expandafter\newcount\csname g__tag_support_tcb/capture/init_args_int\endcsname
\expandafter\newcount\csname g__tag_support_tcb_capture_init_args_int\endcsname
\makeatother
...
% 設定目錄格式 - Adjusted for visibility
...
% --- TCOLORBOX LIBRARIES ---
% Load libraries while spoof is still active!
\tcbuselibrary{skins, breakable, raster} 

% Restore Kernel Check Function
\let\IfFormatAtLeastTF\ORIGIfFormatAtLeastTF
\makeatother 
...
\begin{titlepage}
  \thispagestyle{empty}
  \pagecolor{NavyBlue} % 使用 pagecolor 直接設定整頁背景色 (最穩定)
  ...
  \vspace{1cm}
  {\Large \sffamily \color{ElectricCyan} 台中易起來：在地二手物交易平台 \par}
  \vspace{0.5cm}
  {\large \sffamily \color{white} 逢甲大學資訊工程學系學士後專班 \par}

  \vfill

  \begin{tcolorbox}[colback=white!10!NavyBlue, colframe=ElectricCyan, width=0.8\textwidth, arc=2mm]
    \centering \color{white}
    \textbf{課程：} 軟體品質與測試 (指導教授：薛念林 博士) \\
    \textbf{組別：} 第三組 \\
    \textbf{成員：} 林政佑 / 陶子中 / 游程鈞 / 陳韋翰 \\
...
\subsection{專題目標}
本專案針對「台中易起來 - 在地二手物交易平台」進行單元測試。有鑑於現代前端應用邏輯的複雜性，本報告確立了**以 `src/api` 層為測試核心**的策略，確保資料互動的準確性與穩定性。
...
\subsubsection{系統操作畫面 (System Operation)}
以下展示本平台實際運行之核心功能畫面，包含首頁、地圖、交易流程與個人中心等。

\begin{figure}[H]
  \centering
  \begin{minipage}{0.48\textwidth}
    \centering
    \includegraphics[width=\linewidth]{images/screenshot/home.png}
    \caption{首頁 (Home)}
  \end{minipage}
  \hfill
  \begin{minipage}{0.48\textwidth}
    \centering
    \includegraphics[width=\linewidth]{images/screenshot/map.png}
    \caption{物品地圖搜尋 (Map Search)}
  \end{minipage}
\end{figure}
...
\begin{figure}[H]
  ...
  \begin{minipage}{0.48\textwidth}
    \centering
    \includegraphics[width=\linewidth]{images/screenshot/item_search.png}
    \caption{物品列表與搜尋 (Item Search)}
  \end{minipage}
  \hfill
  \begin{minipage}{0.48\textwidth}
    \centering
    \includegraphics[width=\linewidth]{images/screenshot/item_detail.png}
    \caption{物品詳細資訊 (Item Detail)}
  \end{minipage}
  \vspace{0.5cm}
  \begin{minipage}{0.48\textwidth}
    \centering
    \includegraphics[width=\linewidth]{images/screenshot/item_create.png}
    \caption{物品刊登 (Create Item)}
  \end{minipage}
  \hfill
  \begin{minipage}{0.48\textwidth}
    \centering
    \includegraphics[width=\linewidth]{images/screenshot/item_manage.png}
    \caption{物品管理 (Manage Items)}
  \end{minipage}
\end{figure}
...
\begin{figure}[H]
  ...
  \begin{minipage}{0.31\textwidth}
    \centering
    \includegraphics[width=\linewidth]{images/screenshot/item_transaction.png}
    \caption{交易流程 (Transaction)}
  \end{minipage}
  \hfill
  \begin{minipage}{0.31\textwidth}
    \centering
    \includegraphics[width=\linewidth]{images/screenshot/chat.png}
    \caption{即時通訊 (Chat)}
  \end{minipage}
  \hfill
  \begin{minipage}{0.31\textwidth}
    \centering
    \includegraphics[width=\linewidth]{images/screenshot/user_review.png}
    \caption{用戶評價 (Reviews)}
  \end{minipage}
\end{figure}
...
\begin{figure}[H]
  ...
  \begin{minipage}{0.48\textwidth}
    \centering
    \includegraphics[width=\linewidth]{images/screenshot/user_profile.png}
    \caption{個人檔案 (Profile)}
  \end{minipage}
  \hfill
  \begin{minipage}{0.48\textwidth}
    \centering
    \includegraphics[width=\linewidth]{images/screenshot/item_favorite.png}
    \caption{收藏物品 (Favorites)}
  \end{minipage}
  \vspace{0.5cm}
  \begin{minipage}{0.48\textwidth}
    \centering
    \includegraphics[width=\linewidth]{images/screenshot/points_center.png}
    \caption{點數中心 (Points)}
  \end{minipage}
  \hfill
  \begin{minipage}{0.48\textwidth}
    \centering
    \includegraphics[width=\linewidth]{images/screenshot/badges.png}
    \caption{徽章獎勵 (Badges)}
  \end{minipage}
\end{figure}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.95\textwidth]{images/custom/test_exec.png}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.95\textwidth]{images/custom/coverage_report.png}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.95\textwidth]{images/custom/eslint_report.png}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.95\textwidth]{images/custom/metrics_report.png}
...
\begin{itemize}[label=\textcolor{SuccessGreen}{$\checkmark$}]
  ...
  \item \textbf{程式碼複雜度 (WMC)：} 342 (API層，目標 > 200)
...
\begin{center}
  ...
  \begin{tabular}{p{0.2\textwidth} p{0.45\textwidth} p{0.15\textwidth}}
    ...
    \midrule
    林政佑                      & 測試架構與設計       & 30\%          \\
    \rowcolor{LightGray} 陶子中 & 測試案例改善與文件撰寫   & 30\%          \\
    游程鈞                      & 測試案例改善與簡報撰寫   & 30\%          \\
    \rowcolor{LightGray} 陳韋翰 & 意見分享與現場演示     & 10\%          \\
    \bottomrule
  \end{tabular}
\end{center}
...
\begin{itemize}[label=\textcolor{ElectricCyan}{$\blacksquare$}]
  ...
  \item \textbf{AI 工具使用：} GitHub Copilot 測試程式碼與審查建議
\end{itemize}
