<template>
  <button
    class="report-btn report-btn--user"
 ...
 * 檢舉用戶按鈕元件
 *
 * @description 提供一鍵檢舉用戶功能，點擊後會跳出確認視窗
 *
 * @example
 * <ReportUserButton
...
const handleReport = async () => {
  ...
  if (props.showConfirm) {
    const confirmed = window.confirm('確定要檢舉此用戶嗎？')
    if (!confirmed) return
  }

  // 提交檢舉
  const result = await submitReport({
    report_type: REPORT_TYPES.USER,
